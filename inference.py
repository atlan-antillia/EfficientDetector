...
def visualize_image(filters, # list something like [person, car]
                    image,
                    boxes,
                    classes,
                    scores,
  ...
  img = np.array(image)
  image, detected_objects = vis_utils.visualize_boxes_and_labels_on_image_array(
      filters, #
      img,
      boxes,
      classes,
...
def visualize_image_prediction(filters, #list of classes
                               image,
                               prediction,
  ...
  return visualize_image(filters, image, boxes, classes, scores, label_id_mapping,
                         **kwargs)
...
class InferenceDriver(object):
  ...
  def inference(self, filters, # list of classes
    ...
    with tf.Session() as sess:
      ...
      for i, prediction in enumerate(predictions):
        (img, detected_objects) = visualize_image_prediction(
            filters,
            raw_images[i],
            prediction,
            label_id_mapping=self.label_id_mapping,
            **kwargs)
        if i == 0:
          output_image_path     = os.path.join(output_dir,  basename)
        else:
        ...
        if i == 0:
          detected_objects_path = os.path.join(output_dir, basename + '.txt')
        else:
          detected_objects_path = os.path.join(output_dir, str(i) + '.txt')
